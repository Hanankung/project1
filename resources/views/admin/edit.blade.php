...
<form action="{{ route('admin.update', $post->id)}}" method="POST" enctype="multipart/form-data">
    ...
    <div class="mb-3">
        ...
        <input type="text" class="form-control" name="product_name_ENG" value="{{ $post->product_name_ENG }}" placeholder="Product name in English">
    </div>
    <div class="mb-3">
        <label class="form-label">ชื่อสินค้า (Malay)</label>
        <input type="text" class="form-control" name="product_name_MS" value="{{ $post->product_name_MS }}" placeholder="Product name in Malay">
    </div>
    {{-- รายละเอียดสินค้า --}}
    <div class="mb-3">
    ...
    <div class="mb-3">
        ...
        <input type="text" class="form-control" name="material_ENG" value="{{ $post->material_ENG }}" placeholder="Material in English">
    </div>
    <div class="mb-3">
        <label class="form-label">วัสดุ (Malay)</label>
        <input type="text" class="form-control" name="material_MS" value="{{ $post->material_MS }}" placeholder="Material in Malay">
    </div>
    {{-- ขนาด --}}
    <div class="mb-3">
