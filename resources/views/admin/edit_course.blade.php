...
<form action="{{ route('admin.update_course', $post->id)}}" method="POST" enctype="multipart/form-data">
    ...
    <div class="mb-3">
        ...
        <input type="text" class="form-control" id="course_name_ENG" name="course_name_ENG" value="{{ $post->course_name_ENG}}" placeholder="Course name in English">
    </div>
    <div class="mb-3">
        <label for="course_name_MS" class="form-label">ชื่อคอร์สเรียน (Malay)</label>
        <input type="text" class="form-control" id="course_name_MS" name="course_name_MS" value="{{ $post->course_name_MS}}" placeholder="Course name in Malay">
    </div>
    {{-- รายละเอียด --}}
    <div class="mb-3">
